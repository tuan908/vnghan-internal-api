...
fileRouterV1.post("/excel", async (c) => {
  const db = c.get("db");
...
fileRouterV1.post("/pdf", async (c) => {});

fileRouterV1.post("/importExcel", async (c) => {
  ...
  try {
    ...
    for (const worksheet of workbook.worksheets) {
      ...
      for (let i = startRow; i <= usedRowCount; i++) {
        ...
        if (!quantity || !price) {
          errors.push(`Row ${i} in "${worksheet.name}": missing required data`);
          continue;
        }
    ...
    await invalidateCache(REDIS_URL, REDIS_TOKEN, `GET:/api/v1/screws`);
